...
class PlayerBroadcastBridge(
    ...
    companion object {
        ...
        const val CONTROL_TYPE_SEEK_TO = 7
    }
    ...
    private val receiver = object : BroadcastReceiver() {
        override fun onReceive(ctx: Context?, intent: Intent?) {
            ...
            when (intent.getIntExtra(EXTRA_CONTROL_TYPE, -1)) {
                ...
                CONTROL_TYPE_REWIND -> scope.launch { listener.onSeekRequested(false, 30_000L) }
                CONTROL_TYPE_FORWARD -> scope.launch { listener.onSeekRequested(true, 30_000L) }
                ...
                CONTROL_TYPE_SEEK_TO -> scope.launch {
                    listener.onSeekToRequested(intent.getLongExtra(EXTRA_SEEK_POSITION_MS, 0L))
                }
            }
        }
    }
