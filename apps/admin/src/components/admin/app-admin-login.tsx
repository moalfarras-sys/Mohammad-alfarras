...
export function AppAdminLogin({ message, initialEmail }: { message?: string; initialEmail?: string }) {
  return (
    <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.03)),rgba(6,10,18,0.95)] p-6 shadow-[0_40px_100px_rgba(0,0,0,0.5)] backdrop-blur-3xl sm:p-8 md:p-12">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.16),transparent_40%),radial-gradient(circle_at_bottom_right,rgba(139,92,246,0.16),transparent_38%)]" />

      <div className="relative mx-auto max-w-md space-y-8">
        <div className="flex justify-center">
          <div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-3xl bg-white/5 shadow-2xl ring-1 ring-white/10 backdrop-blur-xl">
            <Image src="/icons/icon-192.png" alt="Moalfarras Admin" width={56} height={56} className="h-14 w-14 rounded-2xl object-cover" />
          </div>
        </div>
        ...
        <div className="space-y-3 text-center">
          ...
          <p className="text-sm leading-7 text-foreground-muted">
            Authenticate to manage the Mohammad Alfarras website, MoPlayer releases, activation flow, support, and app runtime controls.
          </p>
        </div>
        ...
        <form action={loginAdminAction} className="space-y-5">
          ...
          <button type="submit" className="button-primary-shell w-full py-4 text-sm uppercase tracking-widest">
