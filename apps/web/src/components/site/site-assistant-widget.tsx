...
const widgetCopy = {
  en: {
    ...
    title: "Mo Ai",
    status: "Site guide",
    initial: "Hi. I can guide you through this page, MoPlayer, activation, services, and contact options.",
    placeholder: "Ask about this page...",
    send: "Send",
    close: "Close assistant",
    open: "Open Mo Ai assistant",
    thinking: "Thinking...",
    starters: ["What is this page?", "How do I activate MoPlayer Pro?", "How can I contact Mohammad?"],
  },
  ar: {
    label: "Mo Ai",
    title: "Mo Ai",
...
function initialMessages(locale: Locale): Message[] {
  ...
  try {
    ...
    const valid = Array.isArray(rawMessages)
      ? rawMessages.filter((message) => (message.role === "user" || message.role === "assistant") && typeof message.content === "string")
      : [];
...
export function SiteAssistantWidget({ locale }: { locale: Locale }) {
  ...
  const isDedicatedAssistantPage = pathname === `/${locale}/ai` || pathname === "/ai";

  useEffect(() => {
    try {
      const payload: SavedThread = { savedAt: new Date().toISOString(), messages: messages.slice(-10) };
  ...
  async function sendMessage(text: string) {
    ...
    try {
      ...
      setMessages((current) => [...current, { role: "assistant" as const, content: data.reply! }].slice(-10));
    } catch (err) {
  ...
  return (
    <aside className={open ? "mo-ai-widget mo-ai-widget-open" : "mo-ai-widget"} aria-label={t.title as string}>
      {open ? (
        <section className="mo-ai-panel">
          ...
          <div className="mo-ai-thread">
            {messages.map((message, index) => (
              <div key={`${message.role}-${index}`} className={`mo-ai-bubble mo-ai-bubble-${message.role}`}>
                <p>{message.content}</p>
              </div>
            ))}
            {loading ? (
          ...
          >
            <input value={input} onChange={(event) => setInput(event.target.value)} placeholder={t.placeholder as string} />
      ...
      <button type="button" className="mo-ai-fab" onClick={() => setOpen((value) => !value)} aria-label={t.open as string}>
